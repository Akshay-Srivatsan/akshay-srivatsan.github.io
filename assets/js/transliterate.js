...
function fix_ipa(s) {
    s = s.normalize();
    return Array.from(s)
        .map((x, i) => (i > 0 && x === s[i - 1] && x !== ' ' ? 'ː' : x))
        .join('')
        .replaceAll('ː̃', fix_string('◌̃ː'))
        .replaceAll('n̪n̪', 'n̪ː');
}
...
function fix_tamil(s) {
    ...
    for (let i = 0; i < n; i++) {
        ...
        if (c === 'ந' && i > 0) {
            if (
                chars[i - 1] !== ' ' &&
                !(chars[i + 1] === '्' && 'तथदध'.indexOf(chars[i + 2]) === -1)
            ) {
                chars[i] = 'ன';
            }
        }
...
function brahmic_convert(s, from, to) {
    ...
    let set = new Set();
    let entries = brahmic_map
        .map((x) => [fix_string(x[from]), fix_string(x[to])])
        .filter(([k, v]) => {
...
function iso_convert(s, to) {
    ...
    let map = Object.fromEntries(
        ipa_map.map((x) => [fix_string(x[from]), fix_string(x[to])])
    );
